...
from .models import Institucionfinanciera, Departamento, Servicios, PlanesConcretados
...
class ReporteView(LoginRequiredMixin, View):
    ...
    def desdeInstitucion(self,queryset, datos):
        ...
        for institucion in queryset:
            if not datos.get(institucion.idServicio.idDepartamento.idInstitucionFinanciera.nombre) :
                #print("institucion:{}".format(institucion.idServicio.idDepartamento.idInstitucionFinanciera.nombre))
                #Ahora se obtienen todos los departamentos pertenecientes a la institucion indicada
                departamentos = self.desdeDepartamento(queryset.filter(idServicio__idDepartamento__idInstitucionFinanciera__nombre = institucion.idServicio.idDepartamento.idInstitucionFinanciera.nombre ))
                #Se agrega el diccionario de departamentos con planes al diccionario de las instituciones
                datos[institucion.idServicio.idDepartamento.idInstitucionFinanciera.nombre] = departamentos
    ...
    def desdePlan(self, queryset):
        planes = []
        for p in queryset:
            planes.append((p.idServicio.nombrePlan, p.numeroPoliza, p.plazoAnos, p.primaNetaAnual, p.valorPlan))
        ...
        if self.request.session.has_key('institucion'):
            ...
            if(inst == "0"):
                ...
                if(self.request.session.has_key('departamento')):
                    ...
                    if(deptos == "0"):
                        queryset = PlanesConcretados.objects.filter(Q(idAsesor=request.user.id), Q(fechaContratacion__range=[pInicial,pFinal])).filter(idServicio__idDepartamento__idInstitucionFinanciera = inst)
                        #print("NOMBRE INST: {}".format(queryset[0].idServicio.idDepartamento.idInstitucionFinanciera.nombre))
                        #datos[queryset[0].idServicio.idDepartamento.idInstitucionFinanciera.nombre] = self.desdeDepartamento(queryset)
                        self.desdeInstitucion(queryset,datos)
                        ...
                        if(plan == "0"):
                            ...
                            queryset = PlanesConcretados.objects.filter(Q(idAsesor=request.user.id), Q(fechaContratacion__range=[pInicial,pFinal])).filter(idServicio__idDepartamento__idDepartamento = deptos).filter(idServicio__idDepartamento__idInstitucionFinanciera = inst)
                            #queryset1 = PlanesConcretados.objects.filter(idAsesor=request.user.id, idServicio__idDepartamento__idInstitucionFinanciera = inst, idServicio__idDepartamento__idDepartamento = deptos).values()
                            #print("QUERySET!: {}".format(qset))
                            #for x in qset:
                            #    print("NOMBRE INST: {}".format(x.idServicio.idDepartamento.idInstitucionFinanciera.nombre))
                            #    print("NOMBRE DEPTO: {}".format(x.idServicio.idDepartamento.nombre))
                                #datos[x.idServicio.idDepartamento.idInstitucionFinanciera.nombre] = {x.idServicio.idDepartamento.nombre : self.desdePlan(qset)}
                            self.desdeInstitucion(queryset,datos)
                            print("Datos X: {}".format(datos))
                        else:
                            queryset = PlanesConcretados.objects.filter(Q(idAsesor=request.user.id), Q(fechaContratacion__range=[pInicial,pFinal])).filter(idServicio__idDepartamento__idInstitucionFinanciera = inst).filter(idServicio__idDepartamento__idDepartamento = deptos).filter(idServicio = plan)
                            #for p in queryset:
                                #datos[p.idServicio.idDepartamento.idInstitucionFinanciera.nombre] = {p.idServicio.idDepartamento.nombre : self.desdePlan(queryset)}
                            self.desdeInstitucion(queryset, datos)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if(request.is_ajax()):
            if(depto == "0"):
                ...
                plan["planes"] = list(Servicios.objects.filter(idDepartamento__pk=depto).values_list('nombrePlan', 'idServicio'))
        ...
        if(inst != ''):
            ...
            if(request.is_ajax()):
                if(inst == "0"):
                    ...
                    depto["deptos"] = list(Departamento.objects.filter(idInstitucionFinanciera__pk=inst).values_list('nombre', 'idDepartamento'))
